...
| M6 | 测试与加固 | 5 | UI 测试通过；真机矩阵通过；§60 安全项逐条验证 |
...
| T6.2 | 真机矩阵测试 | 按 §4.2 设备逐台执行测试用例表 | M5 | 2 | 所有 P0 用例通过，问题入 issue |
...
注意：华为 HarmonyOS NEXT 不支持安装 Android APK，不在支持范围内；HarmonyOS 4 及以下仍可测试。
...
- [ ] 进程死亡恢复后必须解锁
- [ ] PIN 退避在重启后仍生效
- [ ] 厂商换机 / 本地备份工具不迁移数据，或迁移后进入 `Unreadable` 且不覆盖
